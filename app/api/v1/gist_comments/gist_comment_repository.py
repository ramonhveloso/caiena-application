...
class GistCommentRepository:
    async def create(
        self, db: AsyncSession, user_id: int, comment_id: int, gist_comment: CreateGistCommentRequest
    ) -> GistCommentResponse:
    ...
    async def get_gist_comment_by_id(self, db: AsyncSession, comment_id: int):
        return db.query(GistComment).filter(GistComment.comment_id == comment_id).first()

    async def update(self, db: AsyncSession, gist_comment: GistComment, data: PutGistCommentRequest):
        ...
        gist_comment.forecast_day_1_date = data.forecast_day_1_date if data.forecast_day_1_date else gist_comment.forecast_day_1_date
        gist_comment.forecast_day_1_temperature = data.forecast_day_1_temperature if data.forecast_day_1_temperature else gist_comment.forecast_day_1_temperature
        gist_comment.forecast_day_2_date = data.forecast_day_2_date if data.forecast_day_2_date else gist_comment.forecast_day_2_date
        gist_comment.forecast_day_2_temperature = data.forecast_day_2_temperature if data.forecast_day_2_temperature else gist_comment.forecast_day_2_temperature
        gist_comment.forecast_day_3_date = data.forecast_day_3_date if data.forecast_day_3_date else gist_comment.forecast_day_3_date
        gist_comment.forecast_day_3_temperature = data.forecast_day_3_temperature if data.forecast_day_3_temperature else gist_comment.forecast_day_3_temperature
        gist_comment.forecast_day_4_date = data.forecast_day_4_date if data.forecast_day_4_date else gist_comment.forecast_day_4_date
        gist_comment.forecast_day_4_temperature = data.forecast_day_4_temperature if data.forecast_day_4_temperature else gist_comment.forecast_day_4_temperature
        gist_comment.forecast_day_5_date = data.forecast_day_5_date if data.forecast_day_5_date else gist_comment.forecast_day_5_date
        gist_comment.forecast_day_5_temperature = data.forecast_day_5_temperature if data.forecast_day_5_temperature else gist_comment.forecast_day_5_temperature
        db.commit()
        db.refresh(gist_comment)
        return gist_comment

    async def delete(self, db: AsyncSession, comment_id: int):
        comment_entry = db.query(GistComment).filter(GistComment.comment_id == comment_id).first()
        if comment_entry:
